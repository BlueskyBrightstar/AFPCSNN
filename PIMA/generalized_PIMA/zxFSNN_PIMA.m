%% txFuzzy spiking neural network, PIMA %%
% M = 5; Q = 11; K = 5; L = 12; tau = 7; beta1 = 1.4; b2 = 2; eta = 0.008;

%save('zxFSNN_PIMA_jd.mat')
clear
close all
clc
P = 1;
...
datax1 = pima_indians_diabetes(:,1:8);
...
kkk=zeros(1,5);
npima_indians_diabetes=[datax datay datay1];
indices = crossvalind('Kfold',768, 10);%将数据样本随机分割为10部分
for i = 1:10 %循环10次，分别取出第i部分作为测试样本，其余部分作为训练样本
    test = (indices == i);
    train = ~test;
    trainData = npima_indians_diabetes(train, :);
    x = trainData(:,1:8);
    J = size(x,1);      % the number of training samples
    O = trainData(:,9);
    O1 = trainData(:,10);
    
    testData = npima_indians_diabetes(test, :);
    testx = testData(:,1:8);
    ...
    tmp_ACC = 0;
    tmp_testACC = 0;
    % while epoch<max_epoch && (ACC1<0.78 || testACC1<0.77)
