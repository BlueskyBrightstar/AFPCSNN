%% zxFuzzy spiking neural network, Liver %%
% M = 8; Q = 11; K = 5; L = 12; tau = 7; beta1 = 1.4; b2 = 2; eta = 0.009;

%save('zxFSNN_Liver_jd.mat')
clear
close all
clc
P = 1;
...
kkk=zeros(1,5);
nliver_disorders=[datax datay datay1];
indices = crossvalind('Kfold',345, 10);%将数据样本随机分割为10部分
for i = 1:10 %循环10次，分别取出第i部分作为测试样本，其余部分作为训练样本
    test = (indices == i);
    train = ~test;
    trainData = nliver_disorders(train, :);
    x = trainData(:,1:6);
    J = size(x,1);      % the number of training samples
    O = trainData(:,7);
    O1 = trainData(:,8);
    
    testData = nliver_disorders(test, :);
    testx = testData(:,1:6);
    ...
    M = 8;                     % the number of Gaussian receptive fields
    C = N*M+1;                   % the number of neurons of Fuzzy Coding Layer
    Q = 11; % the number of hidden layer
    ...
    while epoch<max_epoch
        % while epoch<max_epoch && (ACC1<0.71 || testACC1<0.69)
        %%%%%%% training process %%%%%%%%%%%%%%%%%%%%%%%%%%%%
        accuracy = 0;
        for j = 1:J
            tH = 100*ones(1,Q);
