%% zxFuzzy spiking neural network, Iris %%
% M = 6; Q = 10; K = 5; L = 11; tau = 8; beta1 = 1.4; b2 = 2; eta = 0.03;

%save('zxFSNN_Iris_jd.mat')
clear
close all
clc
P = 3;
...
indices = crossvalind('Kfold', 150, 10);%将数据样本随机分割为10部分
    ...
    while epoch<max_epoch
        ...
        for j = 1:J
            ...
            for n = 1:N
                for m = 1:M
                    c = M*(n-1)+m;
                    tF(c) = roundn(T_interval.*(1-fzx(a(c),b1(c),b2,x(j,n))),-1);
                end
            end
            %%%%% Hidden Layer %%%%%%%%%%%%%%%%%%%%%
            for l = 0:0.2:L
            ...
            for n = 1:N
                for m = 1:M
                    c = (n-1)*M+m;
                    D_a(c) = +sum(deltaQ.*sum(w((c-1)*K+1:c*K,:).*D_y((c-1)*K+1:c*K,:),1))*(T_interval.*fzxa(a(c),b1(c),b2,x(j,n)));
                    D_b1(c) = +sum(deltaQ.*sum(w((c-1)*K+1:c*K,:).*D_y((c-1)*K+1:c*K,:),1))*(T_interval.*fzxb1(a(c),b1(c),b2,x(j,n)));
                    %D_b2(c) = +sum(deltaQ.*sum(w((c-1)*K+1:c*K,:).*D_y((c-1)*K+1:c*K,:),1))*(T_interval.*fzxb2(a(c),b1(c),b2(c),x(j,n)));
                end
            end
        ...
        for j = 1:testJ
            ...
            for n = 1:N
                for m = 1:M
                    c = M*(n-1)+m;
                    testtF(c) = roundn(T_interval.*(1-fzx(a(c),b1(c),b2,testx(j,n))),-1);
                end
            end
