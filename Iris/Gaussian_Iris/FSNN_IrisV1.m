%% Fuzzy spiking neural network, Iris %%
% M = 6; Q = 10; K = 5; L = 12; tau = 8; beta = 1.5; eta = 0.02;

%save('FSNN_Iris_jd.mat')
clear
close all
clc
P = 3;
    ...
    while epoch<max_epoch
        ...
        for j = 1:J
            ...
            for n = 1:N
                for m = 1:M
                    c = (n-1)*M+m;
                    D_a(c) = +sum(deltaQ.*sum(w((c-1)*K+1:c*K,:).*D_y((c-1)*K+1:c*K,:),1))*(T_interval-tF(c))*(x(j,n)-a(c))*b(c)^2;
                    D_b(c) = -sum(deltaQ.*sum(w((c-1)*K+1:c*K,:).*D_y((c-1)*K+1:c*K,:),1))*(T_interval-tF(c))*(x(j,n)-a(c))^2*b(c);
                end
            end
